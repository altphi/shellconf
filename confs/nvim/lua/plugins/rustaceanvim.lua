return {
  "mrcjkb/rustaceanvim",
  version = "^5",        -- IMPORTANT: pin to v5+ (or use false for latest)
  lazy = true,
  ft = { "rust" },
  init = function()
    ...
    local function attach_to_running()
      ...
      for line in handle:lines() do
        ...
        local is_binary = pid and (cmdline:find("target/debug/" .. name, 1, true)
                               or  cmdline:find("target/release/" .. name, 1, true))
      ...
      handle:close()
      -- Prefer debug binary > release binary > cargo wrapper.
      table.sort(entries, function(a, b)
        local function score(c)
          if c:find("target/debug/", 1, true) then return 0 end
          if c:find("target/release/", 1, true) then return 1 end
          return 2
        end
      ...
      if #entries == 1 then
        ...
        vim.ui.select(entries, {
          ...
          format_item = function(e) return e.pid .. "  " .. e.cmdline end,
        }, function(choice)
          if choice then go(choice.pid) end
        end)
      end
    end
    ...
    vim.api.nvim_create_autocmd("FileType", {
      ...
      callback = function(ev)
        vim.keymap.set("n", "<leader>dR", function() vim.cmd.RustLsp("debuggables") end,
          { desc = "Rust: Launch debuggable", buffer = ev.buf })
        vim.keymap.set("n", "<leader>dA", attach_to_running,
          { desc = "Rust: Attach to running process", buffer = ev.buf })
      end,
    })
  end,
}
